...
2. 创建 包 package `t.com.novel.book.wire.domain.book.repository` 在原包`com.novel.book.wire.domain.book.repository`加个前缀 `t.`;
3. 添加接口 `interface _BookAuthorRepository` 在原接口 `BookAuthorRepository`前缀  `_` (下划线);
4. 需要扩展 `extends BookAuthorRepository` (方便编辑器提示)
5. 将所有 `BookAuthorRepository` 里面的`@Derived` 方法拷贝一份到 `_BookAuthorRepository`
6. 执行 `stub` 命令
